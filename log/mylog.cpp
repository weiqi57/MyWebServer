...
bool mylog::init(const char* file_name, int close_log, int max_queue_size, int log_buf_size,
                 int log_max_line) {
    // 异步处理
    if (max_queue_size > 0) {
        m_is_async = true;
    ...
    snprintf(log_full_name, 255, "%d-%02d-%02d-%s",
             (sys_time_tm->tm_year) + 1900, (sys_time_tm->tm_mon) + 1,
             sys_time_tm->tm_mday, file_name);
...
void mylog::write_log(int level, const char* format, ...) {
    ...
    if (m_today != sys_time_tm->tm_mday ||
        m_count_lines % m_log_max_line == 0) {
        fflush(m_fp);
        // 关闭旧的文件描述符，需要创建新的日志文件
        fclose(m_fp);

        char tail[16] = {0};
        snprintf(tail, 255, "%d_%02d_%02d_", (sys_time_tm->tm_year) + 1900,
                 (sys_time_tm->tm_mon) + 1, sys_time_tm->tm_mday);
    ...
    int n = snprintf(m_buf, 30, "%d/%02d/%02d %02d:%02d:%02d %s ",
                     (sys_time_tm->tm_year) + 1900, (sys_time_tm->tm_mon) + 1,
                     sys_time_tm->tm_mday, sys_time_tm->tm_hour,
                     sys_time_tm->tm_min, sys_time_tm->tm_sec, s);
    // 内容格式化
